...
export function SettingsPage() {
  ...
  return (
    <div className="h-full w-full flex text-black dark:text-neutral-200" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      {/* Sidebar — transparent so native vibrancy shows through */}
      <div className="w-[180px] shrink-0 bg-transparent flex flex-col p-2 pt-[44px] gap-0.5 border-r border-black/10 dark:border-white/8 drag">
        <h2 className="text-lg font-semibold px-2.5 mb-3 text-black dark:text-neutral-200 no-drag">Settings</h2>
        {tabs.map(tab => (
          <button
            key={tab.id}
            className={`flex items-center h-8 px-2.5 gap-2 rounded-md cursor-pointer shrink-0 transition-colors text-[13px] select-none text-left w-full border-none no-drag
              ${activeTab === tab.id
                ? 'bg-white/60 dark:bg-white/12 text-black dark:text-neutral-200 font-medium'
                : 'bg-transparent text-neutral-500 dark:text-neutral-400 hover:bg-black/5 dark:hover:bg-white/6'
              }`}
      ...
      {/* Content — opaque background */}
      <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-neutral-800">
        <div className="h-[38px] shrink-0 drag" />
        <div className="flex-1 overflow-y-auto px-8 pb-4 scrollbar-thin">
          {activeTab === 'keys' && (
            <>
              <div className="mb-5">
                <label htmlFor="api-key-input" className={labelClass}>OpenAI API Key</label>
                <input type="password" id="api-key-input" className={inputClass} placeholder="sk-..." spellCheck={false} autoComplete="off" value={apiKey} onChange={(e) => setApiKey(e.target.value)} />
                <p className={helpClass}>Your key is stored locally and never shared.</p>
              </div>
              <div className="mb-5">
                <label htmlFor="anthropic-key-input" className={labelClass}>Anthropic API Key</label>
                <input type="password" id="anthropic-key-input" className={inputClass} placeholder="sk-ant-..." spellCheck={false} autoComplete="off" value={anthropicKey} onChange={(e) => setAnthropicKey(e.target.value)} />
                <p className={helpClass}>Required for Claude Opus 4.6. Get a key at console.anthropic.com</p>
              </div>
              <div className="mb-5">
                <label htmlFor="google-key-input" className={labelClass}>Google AI API Key</label>
                <input type="password" id="google-key-input" className={inputClass} placeholder="AI..." spellCheck={false} autoComplete="off" value={googleKey} onChange={(e) => setGoogleKey(e.target.value)} />
                <p className={helpClass}>Required for Gemini 3.1 Pro. Get a key at aistudio.google.com</p>
              </div>
              <div className="mb-5">
                <label htmlFor="brave-key-input" className={labelClass}>Brave Search API Key</label>
                <input type="password" id="brave-key-input" className={inputClass} placeholder="BSA..." spellCheck={false} autoComplete="off" value={braveKey} onChange={(e) => setBraveKey(e.target.value)} />
            ...
            <>
              <div className="mb-5">
                ...
                <div className="flex gap-2">
                  {([
                    { id: 'light', label: 'Light', dot: 'bg-white border border-neutral-300' },
                    { id: 'sunset', label: 'Sunset', dot: 'bg-[#1a1410] border border-[#5a4030]' },
                    { id: 'dark', label: 'Dark', dot: 'bg-neutral-900 border border-neutral-600' },
                    { id: 'system', label: 'System', dot: 'bg-gradient-to-r from-white to-neutral-900 border border-neutral-400' },
                  ] as const).map(opt => (
                    <button
                      key={opt.id}
                      className={`flex-1 flex flex-col items-center gap-2 px-3 py-3 rounded-lg border text-[12px] cursor-pointer transition-colors ${
                        theme === opt.id
                          ? 'border-blue-500 bg-blue-500/10 text-blue-600 dark:text-blue-400 font-medium'
                          : 'border-neutral-200 dark:border-neutral-700 bg-transparent text-neutral-600 dark:text-neutral-400 hover:bg-black/5 dark:hover:bg-white/5'
                      }`}
                      onClick={() => {
                        setTheme(opt.id);
                        (window as any).__applyTheme?.(opt.id);
                        window.browser.setTheme(opt.id);
                      }}
                    >
                      <div className={`w-8 h-8 rounded-full ${opt.dot}`} />
                      {opt.label}
                    </button>
                  ))}
                </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="grid grid-cols-2 gap-2 mb-3">
                        <div>
                          ...
                          <input className={inputClass} value={selectedAccount.label} onChange={e => updateAccount(selectedAccount.id, { label: e.target.value })} placeholder="Work Gmail" />
                        </div>
                        <div>
                          <label className="block text-[11px] font-medium mb-1 text-neutral-500 dark:text-neutral-400">Email</label>
                          <input className={inputClass} value={selectedAccount.email} onChange={e => updateAccount(selectedAccount.id, { email: e.target.value })} placeholder="you@example.com" />
                        </div>
                      </div>
          ...
          {activeTab === 'data' && (
            <>
              <div className="mb-5">
                <label className={labelClass}>Browsing History</label>
                ...
                <button
                  ...
                  onClick={async () => {
                    await window.browser.saveHistory([]);
                    window.browser.closeWindow();
                  }}
                >
                  Clear History
        ...
        {/* Footer */}
        <div className="flex justify-end gap-2 p-5 px-8 pt-4 shrink-0 border-t border-neutral-200 dark:border-neutral-700">
          <button
            ...
            onClick={() => window.browser.closeWindow()}
          >
            Cancel
          </button>
          <button
